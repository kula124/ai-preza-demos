...
export default function StoryLibraryPage() {
  ...
  const handleDelete = async (storyId: number, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
      ...
      <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-2xl overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-purple-500 to-pink-600 p-6">
          <div className="flex items-center justify-between">
            ...
            <Link
              href="/bedtime-story"
              className="bg-pink-500 bg-opacity-20 hover:bg-opacity-30 text-white px-4 py-2 rounded-full flex items-center gap-2 transition-all duration-200"
            >
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {stories.map((story) => (
                <Link
                  key={story.id}
                  href={`/story/${story.id}`}
                  className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-950/20 dark:to-pink-950/20 rounded-xl p-4 border border-purple-200 dark:border-purple-800 hover:shadow-lg transition-all block"
                >
